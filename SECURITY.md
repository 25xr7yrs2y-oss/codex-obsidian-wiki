...
If you find a security issue in codex-obsidian-wiki, please report it privately rather than opening a public issue.

**Preferred:** GitHub's private reporting at the repository's [Security Advisories](../../security/advisories/new) page.

**Alternative:** open a private advisory or contact the fork maintainer. For upstream-only issues, use the upstream project's published security contact.
...
- The Codex prompts under `prompts/`
- The legacy manifests under `.claude-plugin/`
...
codex-obsidian-wiki assumes a **single-tenant** deployment: one user, one vault, one machine. Several design decisions follow from this assumption and would need explicit hardening for multi-tenant or shared-CI scenarios:
...
- **The legacy PostToolUse auto-commit hook** (`hooks/hooks.json`) runs as the user invoking the legacy Claude Code path. It auto-commits `wiki/`, `.raw/`, and `.vault-meta/` paths on every Write/Edit. Set `.vault-meta/auto-commit.disabled` (any contents) to opt out per-vault. For shared repos, prefer disabling the hook entirely or using a more restrictive commit policy.
