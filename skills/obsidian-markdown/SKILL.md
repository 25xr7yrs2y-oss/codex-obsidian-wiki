...
**Substrate preference (v1.7+)**: This skill is a self-contained fallback. **Prefer `kepano/obsidian-skills`** (by Steph Ango, Obsidian CEO) as the authoritative substrate when it is available — its `obsidian-markdown` skill is the canonical Obsidian syntax reference for portable agent runtimes. The reference below keeps this vault functional without any external skill install. Repo: [github.com/kepano/obsidian-skills](https://github.com/kepano/obsidian-skills).
